...
- The [operational model detailed in Chapter 4](chapter4.md)
- The [technical framework specified in Chapter 8](chapter8.md)
- The [trust model outlined in Section 4.1](chapter4.md#41-trust-model-and-governance-framework)
...
This category includes scenarios related to how educational institutions and professional bodies issue digital credentials. It covers the process from the completion of educational and professional requirements to the secure issuance of a digital credential. These use cases will demonstrate how the framework ensures the authenticity and integrity of newly issued credentials.

#### 2. Credential Verification

These use cases focus on how employers, educational institutions, and other authorised parties can verify credentials. They will showcase the efficiency and reliability of the verification process compared to traditional methods. Scenarios in this category will address both domestic and cross-border verification challenges.

#### 3. Learner Empowerment

Use cases in this area will illustrate how learners can manage and share their credentials. They will demonstrate the increased control and privacy that learners have over their educational data. Scenarios will include how learners can compile and present their qualifications for various purposes.

#### 4. Lifelong Learning Support

These use cases will show how the framework supports the accumulation and recognition of diverse learning experiences. They will cover scenarios involving micro-credentials, non-formal learning, and continuous professional development. The focus will be on how the framework facilitates a comprehensive view of an individual's learning journey.

#### 5. Cross-Border Mobility

This category will include scenarios that demonstrate how the framework facilitates studying or working in different EU countries. Use cases will show how the framework simplifies the recognition of qualifications across borders. They will address challenges related to different educational and professional systems and qualification frameworks.

#### 6. Fraud Prevention and Detection

These use cases will illustrate how the framework prevents credential fraud and detects fraudulent activities. They will demonstrate the security features of the system and how they protect the integrity of credentials. Scenarios will include how the system handles credential revocation and updates.

#### 7. Institutional Integration

Use cases in this category will show how educational institutions can integrate the framework into their existing systems. They will cover scenarios of migration from traditional to digital credential systems. These use cases will address challenges and solutions in adopting the new framework.

#### 8. Employer and Recruitment Scenarios

These will focus on how the framework streamlines recruitment processes for employers. Use cases will demonstrate how the system facilitates more efficient and accurate candidate assessment. They will show how the framework supports skills-based recruitment and talent mobility.

#### 9. Data Analytics and Policy Making

This category will include scenarios showing how the framework can provide insights for educational policy making. Use cases will demonstrate how aggregated, anonymised data can inform curriculum development and labour market alignment. They will address how the system maintains privacy while providing valuable analytical insights.

#### 10. Continuous Professional Development

These use cases will focus on how the framework supports ongoing professional learning and certification. They will demonstrate how professional bodies can issue and manage credentials for continuing education. Scenarios will include how professionals can showcase their up-to-date qualifications and skills.
...
- **The story**: Narrative description
...
- **Trust infrastructure**: This technology stores the governance, schemas, and other metadata associated with the use case. It provides a secure, tamper-proof record of the credential ecosystem without storing personal data on-chain. The actual credentials are issued directly from the issuer to the student's wallet.
...
- **Distributed verification capabilities**: The trust infrastructure must provide distributed verification capabilities, redundant record keeping, independent verification pathways, and protection against single points of failure.
...
|-------------------|-------|--------|-------------------|
| 1. Student obtains verifiable credentials | Student (Eva) | Requests EducationalID and MyAllianceID | Home University (Rovira i Virgili) issues credentials directly to Eva's EUDI wallet |
| | Home University (Rovira i Virgili) | Verifies Eva's enrolment status | University Student Information System |
| | Home University (Rovira i Virgili) | Issues EducationalID and MyAllianceID | Credential Issuance System |
| 2. Student applies for a course at an alliance university | Student (Eva) | Selects course at University Duisburg-Essen | Alliance Portal |
| | Student (Eva) | Submits application using digital credentials | Eva's Digital Wallet |
| | Host University (Duisburg-Essen) | Verifies Eva's credentials | Credential Verification System |
| | Host University (Duisburg-Essen) | Processes application | University Admissions System |
| 3. Student completes the course | Student (Eva) | Attends and completes course | Learning Management System at University Duisburg-Essen |
| | Host University (Duisburg-Essen) | Records course completion | University Academic Records System |
| 4. Student receives a verifiable credential for course completion | Host University (Duisburg-Essen) | Issues verifiable Transcript of Records | Credential Issuance System |
| | Student (Eva) | Receives credential in EUDI wallet | Eva's Digital Wallet |
| 5. Student shares credentials with home university | Student (Eva) | Selects and shares new credential | Eva's Digital Wallet |
| | Home University (Rovira i Virgili) | Verifies received credential | Credential Verification System |
| | Home University (Rovira i Virgili) | Updates Eva's academic record | University Academic Records System |
...
- **Trust infrastructure**: To store the trust framework and credential metadata, ensuring a tamper-proof record of the credential ecosystem.
...
|-------------------|-------|--------|--------------------|
| **1. Obtaining credentials** | | | |
| 1a. Access eDiplomas platform | Anna (Student) | Visits the eDiplomas platform | eDiplomas web interface |
...
Professional qualification management delivers benefits across three strategic areas:
...
- **Trust infrastructure**: Must provide distributed verification capabilities, redundant record keeping, independent verification pathways, and protection against single points of failure to ensure the immutability and traceability of credential issuance and verification.
...
| User Journey Step | Actor | Action | System Interaction |
|-------------------|-------|--------|-------------------|
| 1. Request eCIP | Anna (Collegiate doctor) | Accesses COM's platform and submits eCIP request | COM digital platform; Doctor selects eCIP request option and provides identification |
| | COM administrative staff | Receive request and authenticate on CGCOM platform | COM staff access CGCOM platform and authenticate |
| 2. Verify credentials | CGCOM | Verifies Anna's credentials | CGCOM platform queries Central Registry to check registration and disqualifications |
| 3. Issue eCIP | CGCOM | Approves request and issues eCIP | CGCOM administrative platform generates and stores eCIP |
| | CGCOM administrative staff | Confirm details and trigger eCIP release | Staff access issuance system, review details, and release eCIP |
| 4. Provide eCIP | CGCOM | Notifies Anna that eCIP is ready | CGCOM system sends notification to Anna's EUDI Wallet |
| | Anna (Collegiate doctor) | Receives notification and retrieves eCIP | EUDI Wallet app; Doctor authenticates and receives eCIP |
| 5. Share eCIP | Anna (Collegiate doctor) | Shares eCIP when required | EUDI Wallet app; Doctor selects and sends eCIP |
| | Receiving institution (e.g., foreign regulatory body) | Receives and verifies eCIP | Institution's system verifies eCIP through CGCOM Central Registry |
...
1. Non-collegiate doctor requests eCIP through CGCOM platform
2. CGCOM verifies doctor's qualifications and background
3. CGCOM issues eCIP
4. Doctor receives eCIP in EUDI Wallet
5. Doctor shares eCIP with relevant organisations
...
| User Journey Step | Actor | Action | System Interaction |
|-------------------|-------|--------|-------------------|
| 1. Request eCIP | Dr. James (Non-collegiate doctor) | Accesses CGCOM platform and submits eCIP request | CGCOM platform; Doctor provides personal details and qualifications |
| 2. Verify qualifications | CGCOM | Queries Spanish Official Degree Data Verification Service | Automated verification of academic credentials |
| | CGCOM | Queries Spanish Consultation Service for Criminal Records | Automated background check |
| 3. Issue eCIP | CGCOM | Generates and digitally signs eCIP after successful verification | CGCOM system creates secure digital certificate |
| 4. Provide eCIP | CGCOM | Sends eCIP to Dr. James' EUDI Wallet | Direct transfer to wallet with notification |
| | Dr. James (Non-collegiate doctor) | Receives eCIP in wallet | EUDI Wallet app; Doctor views and manages certificate |
| 5. Share eCIP | Dr. James (Non-collegiate doctor) | Shares eCIP when required | EUDI Wallet app; Doctor selects and sends eCIP |
| | Foreign medical institution | Receives and verifies eCIP | Institution's system verifies through CGCOM Central Registry |

![Image11](./images/bbp-image11.png)

This user journey demonstrates the step-by-step process of obtaining and using an eCIP for a non-collegiate doctor, from submitting the initial request to sharing the certificate with relevant organisations. It shows the interactions between the non-collegiate doctor (Dr. James), CGCOM, verification services, and the receiving institutions, as well as the various systems involved in the streamlined digital process.
...
- Trust infrastructure Registry: A registry used to verify the training provider's credentials.
...
The evolution of use cases should be guided by:

- **Stakeholder feedback**: Regular input from educational institutions, professionals, employers, and students
- **Technological advancement**: Incorporation of new technologies and standards as they become available
- **Regulatory changes**: Adaptation to new European and national regulations affecting digital credentials
- **Market needs**: Response to changing educational and professional mobility requirements

### <a id="_Toc182376699"></a><a id="_Toc184710060"></a>7.9 Conclusion

The use cases presented in this chapter serve as a powerful tool for demonstrating the value and functionality of the trust framework. They provide concrete examples of how the system transforms credential management, enhances educational mobility, and supports lifelong learning and professional qualifications across Europe.

By addressing real-world scenarios, these use cases play a crucial role in:

- **Gaining stakeholder buy-in**: Demonstrating clear benefits for all parties involved in the credential ecosystem
- **Guiding implementation**: Providing practical examples that inform system development and deployment
- **Supporting policy development**: Offering evidence-based insights for regulatory and policy decisions
- **Facilitating adoption**: Showing clear pathways for institutions and professionals to embrace digital credentials

The successful implementation of the framework depends on understanding and addressing the diverse needs illustrated in these use cases. Through continued development and refinement of these scenarios, the European digital credential ecosystem can achieve its goal of supporting seamless educational and professional mobility while maintaining the highest standards of trust, security, and privacy.

As Europe moves towards a more integrated digital education space, these use cases will continue to evolve, reflecting new opportunities and challenges in the digital credential landscape. The framework's flexibility and adaptability, as demonstrated through these diverse scenarios, positions it well to support Europe's educational and professional qualification objectives for years to come.

## JSON Schema Examples

To support the technical implementation of the use cases described above, here are examples of the key JSON schemas referenced throughout this chapter:

### Educational ID Credential Schema
...
```json
{
  ...
  "id": "https://university.example.edu/credentials/educational-id/123456",
  "type": ["VerifiableCredential", "EducationalID"],
  "issuer": "did:web:university.example.edu",
  "issuanceDate": "2024-01-15T10:30:00Z",
  "credentialSubject": {
    "id": "did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK",
    "identifier": "student.123456@university.example.edu",
    "familyName": "Rossi",
    "firstName": "Maria",
    "dateOfBirth": "1998-05-15",
    "schacHomeOrganization": "university.example.edu",
    "eduPersonPrincipalName": "maria.rossi@university.example.edu",
    "eduPersonScopedAffiliation": ["student@university.example.edu"],
    "eduPersonAffiliation": ["student", "member"]
  }
}
```

### Professional Certificate Schema
...
```json
{
  ...
  "id": "https://cgcom.example.org/credentials/ecip/789123",
  "type": ["VerifiableCredential", "ProfessionalCertificate", "eCIP"],
  "issuer": "did:web:cgcom.example.org",
  "issuanceDate": "2024-02-20T14:45:00Z",
  "expirationDate": "2030-02-20T14:45:00Z",
  "credentialSubject": {
    "id": "did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK",
    "professionalID": "MD789123",
    "fullName": "Dr. Anna Rodriguez",
    "profession": "Medical Doctor",
    "specialization": "Cardiology",
    "licenseNumber": "MD-ES-789123",
    "issuingAuthority": "Colegio Oficial de Médicos de Madrid",
...
```json
{
  ...
  "id": "https://university.athens.gr/credentials/degree/456789",
  "type": ["VerifiableCredential", "EuropeanDigitalCredential"],
  "issuer": "did:web:university.athens.gr",
  "issuanceDate": "2024-06-30T16:00:00Z",
  "credentialSubject": {
    "id": "did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK",
    "achievement": {
      "type": "Qualification",
      "title": {
        ...
        "el": "Πτυχίο Επιστήμης Υπολογιστών"
      },
      "educationLevel": "6",
      "eqfLevel": "6",
      "thematicArea": "Information and Communication Technologies",
      "creditPoints": [
        {
          "framework": "ECTS",
          "point": "240"
...
### SEAFORMEC Accreditation Certificate Schema
...
```json
{
  ...
  "id": "https://seaformec.example.org/credentials/accreditation/ACC-2024-001",
  "type": ["VerifiableCredential", "AccreditationCertificate"],
  "issuer": "did:web:seaformec.example.org",
  "issuanceDate": "2024-03-15T09:00:00Z",
  "expirationDate": "2027-03-15T09:00:00Z",
  "credentialSubject": {
    "id": "did:web:medeuco.example.com",
    "organizationName": "MedEduCo Training Solutions",
    "accreditationType": "Continuous Professional Development",
    "programTitle": "Advanced Cardiology Techniques 2024",
    "programDuration": "40 hours",
    "accreditationScope": "Healthcare professionals - Cardiology specialization",
    "assessmentDate": "2024-03-10",
    "assessors": [
      {
        "name": "Prof. Dr. Maria Santos",
        "organization": "SEAFORMEC",
        "expertise": "Medical Education"
      },
      {
        "name": "Dr. Klaus Weber",
        "organization": "UEMS",
        "expertise": "Cardiology Training"
      }
    ],
...
```json
{
  ...
  "id": "https://cgcom.example.org/credentials/bqm/BQM-2024-789456",
  "type": ["VerifiableCredential", "BQMCertificate"],
  "issuer": "did:web:cgcom.example.org",
  "issuanceDate": "2024-04-20T11:30:00Z",
  "credentialSubject": {
    "id": "did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK",
    "participantName": "Dr. Sofia Martinez",
    "professionalID": "MD456789",
    "trainingTitle": "Basic Quality Management in Healthcare",
    "trainingProvider": "Foundation Virtual Campus",
    "trainingDate": "2024-04-15",
    "trainingDuration": "8 hours",
    "completionStatus": "Successfully completed",
    "accreditationBody": "SEAFORMEC",
    "cpdCredits": "8 CPD credits"
  }
}
```

These JSON examples demonstrate the practical implementation of the W3C Verifiable Credentials Data Model and European Learning Model standards referenced throughout the use cases. They provide concrete technical guidance for developers implementing the trust framework described in this chapter.

The schemas follow established standards while incorporating education and healthcare-specific attributes that enable:

- **Cross-border recognition**: Standardised formats that work across European systems
- **Selective disclosure**: Ability to share only necessary information
- **Cryptographic security**: Tamper-evident credentials with digital signatures
- **Interoperability**: Compatibility with existing European educational frameworks
- **Privacy protection**: GDPR-compliant data handling and user control

For complete schema definitions and additional examples, please refer to [Annex C: Data Models](annexes/AnnexC/README.md) and [Chapter 9: Data Models](chapter9.md).

## Implementation Notes

When implementing these use cases, developers and system integrators should consider:

### Technical Implementation Guidelines

1. **Standards Compliance**
   - Ensure full compliance with W3C Verifiable Credentials Data Model 2.0
   - Implement European Learning Model (ELM) specifications correctly
   - Follow eIDAS 2.0 requirements for electronic attestations of attributes

2. **Security Considerations**
   - Implement strong cryptographic signatures for all credentials
   - Use secure communication protocols (TLS 1.3 or higher)
   - Ensure proper key management and rotation procedures
   - Implement audit trails for all credential operations

3. **Privacy by Design**
   - Minimise data collection and processing
   - Implement selective disclosure capabilities
   - Ensure user consent mechanisms are robust
   - Provide clear data retention and deletion policies

4. **Interoperability Requirements**
   - Test integration with multiple wallet providers
   - Ensure compatibility with different national systems
   - Implement standardised APIs for credential exchange
   - Support multiple credential formats where necessary

### Deployment Considerations

1. **Phased Rollout**
   - Start with pilot implementations in controlled environments
...
2. **User Training and Support**
   - Develop comprehensive training materials for all user types
   - Provide multilingual support for European deployment
   - Establish help desk services for technical support
   - Create user guides and documentation

3. **Monitoring and Maintenance**
   - Implement comprehensive logging and monitoring systems
   - Set up alerting for system issues and security events
   - Plan for regular system updates and security patches
   - Establish performance benchmarks and monitoring

These implementation notes complement the use cases by providing practical guidance for turning the conceptual framework into working systems that serve the needs of European education and professional qualification stakeholders.

The successful implementation of these use cases will contribute significantly to achieving the goals of the European Education Area and supporting the digital transformation of credential management across Europe.